module fast_brief_integrated #(
    ...
    input  logic [7:0] num_frames,
    input  logic [15:0] max_corners,
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (vsync_in && !vsync_r) begin
                send_eof <= 1;
            end else if (m_axis_tready) begin // Clear flag once accepted
                send_eof <= 0;
                if (send_eof) begin // After the end of frame increase counter
                    if (frame_cnt < num_frames-1) begin
    ...
    always_comb begin
        ...
        if (m_axis_tready) begin
            ...
            if (frame_cnt != 0) begin
                ...
                m_axis_tlast  = 0;
                m_axis_tdata  = 0;
            end else if (send_eof) begin
                m_axis_tvalid = 1;
                m_axis_tlast  = 1;
                m_axis_tdata  = 0; 
            end else if (descriptor_valid && (corner_cnt < max_corners)) begin
                m_axis_tvalid = 1;
                m_axis_tlast  = 0;
